<div class="text-center grid grid-cols-1 gap-2 print:absolute print:bottom-0 print:w-full">
  <div>
    <p class="print:text-xs">
      Créé avec à
      <a href="https://kit.svelte.dev/" target="_blank" class="text-blue-400">SvelteKit</a>
      & <a href="https://tailwindcss.com/" target="_blank" class="text-blue-400">TailwindCSS</a>.
    </p>
    <p class="text-xs">
      Trouvez le code sur GitHub : <a href="https://github.com/scorsi/resume" class="text-blue-400">github.com/scorsi/resume</a>.
    </p>
  </div>

  <div class="text-center text-xs print:hidden">
    Ce CV est imprimable/téléchargeable en PDF.
  </div>

  <div class="hidden text-xs print:block">
    Ce CV est visionnable en HTML / CSS (préférable) : <a href="https://scorsi.github.io/resume" target="_blank" class="text-blue-400">scorsi.github.io/resume</a>.
  </div>
</div>